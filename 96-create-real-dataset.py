...
def get_motors(with_velocities=False):
...
for idx in range(len(real_imgs[RECORDING])):
    ...
    if idx < len(real_imgs[RECORDING]) - 1:
        current_real_img = real_img_reshaping(real_imgs[RECORDING][idx, :, :, :3])
        next_real_img = real_img_reshaping(real_imgs[RECORDING][idx + 1, :, :, :3])
        ...
        next_pos, next_vel = get_motors(with_velocities=True)
