...
for FILE_IDX in range(149):
    print ("============== file idx: ", FILE_IDX)
    FILE_NAME = FILE_PREFIX + "data_dump_{}.npz".format(FILE_IDX)
    FILE_NAME_OUT = FILE_PREFIX + "data_dump_{}_aligned.npz".format(FILE_IDX)
    ...
    for RECORDING in tqdm(range(10)):
        ...
        print ("\nDBG: Mappings in recording {}: {}"
               "".format(RECORDING, len(mapping_clean)))
        no_of_mappings.append(len(mapping_clean))
...
no_of_mappings = np.array(no_of_mappings)
print ("min/max no of mappings: {}/{} at recordings: {}/{}".format(
    no_of_mappings.min(),
    no_of_mappings.max(),
    no_of_mappings.argmin(),
    no_of_mappings.argmax()
))